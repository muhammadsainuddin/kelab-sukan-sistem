<template>
  <div class="flex flex-col h-full space-y-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-200 shrink-0 border-l-8 border-[#E30613]">
      <div>
        <h1 class="text-2xl font-black text-gray-900 uppercase italic">Penamatan <span class="text-[#E30613]">Keahlian</span></h1>
        <p class="text-xs text-gray-500 font-bold uppercase tracking-widest mt-1">Jumlah Permohonan: {{ senarai.length }} Rekod</p>
      </div>
      <button @click="fetchSenarai" class="px-6 py-2.5 bg-white hover:bg-[#060d0b] hover:text-white border border-gray-300 hover:border-black text-gray-700 text-[9px] font-black uppercase tracking-widest rounded-lg shadow-sm transition-all inline-flex items-center gap-2 active:scale-95">
      ...
      <div class="overflow-y-auto flex-1 custom-scrollbar">
        <table class="w-full text-left text-sm relative border-collapse">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="senarai.length === 0"><td colspan="4" class="px-6 py-10 text-center text-[#E30613] font-bold uppercase tracking-widest text-xs">Tiada permohonan berhenti.</td></tr>
            ...
            <tr v-for="b in senarai" :key="b.id" class="hover:bg-gray-50 transition-colors group">
              <td class="px-6 py-4">
                ...
                <p class="text-[9px] text-gray-400 font-bold mt-1 tracking-widest">MOHON: {{ formatTarikh(b.tarikh_mohon) }}</p>
              </td>
              <td class="px-6 py-4">
                <p class="text-xs text-gray-600 font-medium max-w-xs whitespace-normal line-clamp-3 italic bg-gray-50 p-2 rounded border border-gray-100">"{{ b.sebab_berhenti || 'Tiada alasan diberikan' }}"</p>
              </td>
              <td class="px-6 py-4 text-center">
                <span v-if="b.status_permohonan === 'MENUNGGU'" class="px-2 py-1 text-amber-700 bg-amber-50 border-amber-200 text-[9px] font-black uppercase tracking-widest rounded border shadow-sm inline-block">Memproses</span>
                <span v-else-if="b.status_permohonan === 'LULUS'" class="px-2 py-1 text-red-700 bg-red-50 border-red-200 text-[9px] font-black uppercase tracking-widest rounded border shadow-sm inline-block">Telah Dibuang</span>
                <span v-else class="px-2 py-1 text-[#70B130] bg-green-50 border-green-200 text-[9px] font-black uppercase tracking-widest rounded border shadow-sm inline-block">Kekal Aktif</span>
              </td>
              <td class="px-6 py-4 text-right">
                <div v-if="b.status_permohonan === 'MENUNGGU'" class="flex justify-end gap-2">
                  <button @click="kemaskiniStatus(b.id, b.no_kp, 'LULUS')" :disabled="isProcessing === b.id" class="px-4 py-2 bg-[#E30613] hover:bg-red-700 text-white text-[9px] font-black uppercase tracking-widest rounded shadow-sm transition-all disabled:opacity-50 italic active:scale-95">Sahkan Berhenti</button>
                  <button @click="kemaskiniStatus(b.id, b.no_kp, 'DITOLAK')" :disabled="isProcessing === b.id" class="px-4 py-2 bg-white border border-gray-300 hover:border-black text-gray-700 text-[9px] font-black uppercase tracking-widest rounded shadow-sm transition-all disabled:opacity-50 active:scale-95">Batalkan</button>
                </div>
                <span v-else class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Selesai</span>
              </td>
            </tr>
          </tbody>
        </table>
